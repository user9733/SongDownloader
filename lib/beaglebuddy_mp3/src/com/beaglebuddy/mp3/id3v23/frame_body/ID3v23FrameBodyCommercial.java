 ...
 * </table>
 * <p class="beaglebuddy">
 * A <i>commercial</i> frame body is associated with an ID3v2.3 {@link com.beaglebuddy.mp3.enums.FrameType#COMMERCIAL COMR} {@link com.beaglebuddy.mp3.id3v23.ID3v23Frame frame} which allows vendors to include a price offer in an .mp3 file.
 ...
 *    </thead>
 *    <tbody>
 *       <tr><td class="beaglebuddy">1. </td><td class="beaglebuddy">{@link com.beaglebuddy.mp3.enums.Encoding encoding}         </td><td class="beaglebuddy">character set used to {@link #setEncoding(Encoding) encode} the <i>seller</i> and <i>description</i> fields.                                                          </td></tr>
 *       <tr><td class="beaglebuddy">2. </td><td class="beaglebuddy">prices                                                      </td><td class="beaglebuddy">the purchase {@link com.beaglebuddy.mp3.pojo.Price price} of the .mp3 file.  The purchase price may be specified in equivalent currencies.  For
 ...
 * </p>
 * <p>
 * There may be more than one <i>commercial</i> frame in an ID3v2.3 {@link com.beaglebuddy.mp3.id3v23.ID3v23Tag tag}, but only one with the same seller.
 * </p>
...
public class ID3v23FrameBodyCommercial extends ID3v23FrameBody
{
   ...
   public void parse() throws IllegalArgumentException
   {
      ...
      nextNullTerminatorIndex = getNextNullTerminator(nullTerminatorIndex, Encoding.ISO_8859_1.getCharacterSet());
   ...
   public void setBuffer()
   {
      if (isDirty())
      {
         byte[] priceBytes           = stringToBytes(Encoding.ISO_8859_1.getCharacterSet(), pricesToString(prices));
         byte[] validUntilBytes      = stringToBytes(Encoding.ISO_8859_1.getCharacterSet(), validUntil     );
         byte[] contactURLBytes      = stringToBytes(Encoding.ISO_8859_1.getCharacterSet(), contactURL     );
         byte[] sellerBytes          = stringToBytes(encoding           .getCharacterSet(), seller         );
         byte[] descriptionBytes     = stringToBytes(encoding           .getCharacterSet(), description    );
         byte[] pictureMimeTypeBytes = stringToBytes(Encoding.ISO_8859_1.getCharacterSet(), pictureMimeType);
         int    index                = 0;
