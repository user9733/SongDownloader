 ...
 * </table>
 * <p class="beaglebuddy">
 * A <i>synchronized lyrics/text</i> frame body is associated with an ID3v2.3 {@link com.beaglebuddy.mp3.enums.FrameType#SYNCHRONIZED_LYRIC_TEXT SYLT} {@link com.beaglebuddy.mp3.id3v23.ID3v23Frame frame} which is used to include the words or lyrics in
 ...
 *    </thead>
 *    <tbody>
 *       <tr><td class="beaglebuddy">1. </td><td class="beaglebuddy">{@link com.beaglebuddy.mp3.enums.Encoding encoding}                </td><td class="beaglebuddy">character set used to {@link #setEncoding(Encoding) encode} the <i>description</i> and the <i>text</i>
 *                                                                                                                                                                   fields.                                                                                               </td></tr>
 *       <tr><td class="beaglebuddy">2. </td><td class="beaglebuddy">{@link com.beaglebuddy.mp3.enums.Language language}                </td><td class="beaglebuddy">{@link #setLanguage(Language) language} the <i>synchronized lyrics</i> are written in.                </td></tr>
 *       <tr><td class="beaglebuddy">3. </td><td class="beaglebuddy">{@link com.beaglebuddy.mp3.enums.TimeStampFormat time stamp format}</td><td class="beaglebuddy">{@link #setTimeStampFormat(TimeStampFormat) units} of the <i>time stamp</i> field
 ...
 * </p>
 * <p class="beaglebuddy">
 * There may be more than one <i>synchronized lyrics/text</i> frame in an ID3v2.3 {@link com.beaglebuddy.mp3.id3v23.ID3v23Tag tag}, but only one with the same <i>language</i> and
 ...
 * import com.beaglebuddy.mp3.enums.FrameType;
 * import com.beaglebuddy.mp3.enums.SynchronizedLyric;
...
public class ID3v23FrameBodySynchronizedLyricsText extends ID3v23FrameBody
{
   ...
   public void parse() throws IllegalArgumentException
   {
      ...
      try
      {
         setTimeStampFormat(TimeStampFormat.getTimeStampFormat(buffer[4]));
      }
      catch (IllegalArgumentException ex)
      ...
      nullTerminatorIndex  = getNextNullTerminator(6, encoding.getCharacterSet());
   ...
   /**
    * gets the {@link com.beaglebuddy.mp3.enums.Language language} the synchronized lyrics are written in.
    * @return the {@link com.beaglebuddy.mp3.enums.Language language} the synchronized lyrics are written in.
   ...
   /**
    * sets the {@link com.beaglebuddy.mp3.enums.Language language} the synchronized lyrics are written in.
    * @param language   {@link com.beaglebuddy.mp3.enums.Language language} the synchronized lyrics are written in.
    * @see #getLanguage()
    */
   ...
   public void setBuffer()
   {
      if (isDirty())
      {
         ...
         for(SynchronizedLyric synchronizedLyric : synchronizedLyrics)
            numSynchronizedLyricBytes += stringToBytes(encoding.getCharacterSet(), synchronizedLyric.getText()).length + 4;
         ...
         {
            numSynchronizedLyricBytes += stringToBytes(encoding.getCharacterSet(), synchronizedLyric.getText()).length + 4;
            textBytes = stringToBytes(encoding.getCharacterSet(), synchronizedLyric.getText());
