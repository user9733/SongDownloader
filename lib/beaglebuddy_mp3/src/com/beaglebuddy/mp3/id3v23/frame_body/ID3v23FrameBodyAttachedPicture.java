 ...
 * </table>
 * <p class="beaglebuddy">
 * An <i>attached picture</i> frame body is associated with an ID3v2.3 {@link com.beaglebuddy.mp3.enums.FrameType#ATTACHED_PICTURE APIC} {@link com.beaglebuddy.mp3.id3v23.ID3v23Frame frame} which is used to include pictures in the .mp3 file.
 ...
 *    </thead>
 *    <tbody>
 *       <tr><td class="beaglebuddy">1. </td><td class="beaglebuddy">{@link com.beaglebuddy.mp3.enums.Encoding encoding}                          </td><td class="beaglebuddy">character set used to {@link #setEncoding(Encoding) encode} the <i>description</i> field.                                                                              </td></tr>
 ...
 *       <tr><td class="beaglebuddy">3. </td><td class="beaglebuddy">{@link com.beaglebuddy.mp3.enums.PictureType pictureType}                    </td><td class="beaglebuddy">indicates which of the 21 pre-defined {@link #setPictureType(PictureType) picture types} the attached image is.                                                        </td></tr>
 ...
 * </p>
 * <p class="beaglebuddy">
 * There may be more than one <i>attached picture</i> frame in an ID3v2.3 {@link com.beaglebuddy.mp3.id3v23.ID3v23Tag tag}, but only one with the same <i>description</i> field.
 * There may only be one <i>attached picture</i> frame with the picture type declared as picture type {@link com.beaglebuddy.mp3.enums.PictureType#SMALL_ICON SMALL_ICON} and
 * {@link com.beaglebuddy.mp3.enums.PictureType#OTHER_ICON OTHER_ICON} respectively.
 * </p>
 * <p>
 * example:
 * <code>
 ...
 *          Vector&lt;ID3v23Frame&gt; pictureFrames = new Vector&lt;ID3v23Frame&gt;();
 *
 *          for(ID3v23Frame frame : frames)
 *             if (frame.getHeader().getFrameType().equals(FrameType.ATTACHED_PICTURE))
 *                pictureFrames.add(frame);
 *
 *          for(ID3v23Frame frame : pictureFrames)
 *          {
 *             // for each picture frame, get the frame body (the actual picture data is stored in the frame's body)
 ...
 *             System.out.println("picture type = " + frameBody.getPictureType() + "   -    " + frameBody.getDescription());
 *          }
 *
 *
 *          // load an image of the band's logo from your hard drive
 *          File            imageFile = new File("c:/music/bon jovi/logo.jpg");
 ...
 *          }
 *
 *          // add a picture of the band's logo to the ID3v2.3 tag
 *          ID3v23Frame frame = new ID3v23Frame(FrameType.ATTACHED_PICTURE, new ID3v23FrameBodyAttachedPicture(Encoding.ISO_8859_1, "image/jpg", PictureType.BAND_LOGO, "band logo", image));
 *          frames.add(frame);
 *          mp3.save();                  // save the ID3v2.3 tag to the .mp3 file
...
public class ID3v23FrameBodyAttachedPicture extends ID3v23FrameBody
{
    ...
    * @param file           input stream pointing to a attached picture frame body in the .mp3 file.
    ...
    */
   public ID3v23FrameBodyAttachedPicture(InputStream file, int frameBodySize) throws IOException
   {
      super(file, FrameType.ATTACHED_PICTURE, frameBodySize);
   }
   ...
   public void parse() throws IllegalArgumentException
   {
      ...
      nullTerminatorIndex = getNextNullTerminator(1, Encoding.ISO_8859_1.getCharacterSet());
      setMimeType(new String(buffer, 1, nullTerminatorIndex - 1, Encoding.ISO_8859_1.getCharacterSet()).trim());
      try
      {
         setPictureType(PictureType.getPictureType(buffer[nullTerminatorIndex + 1]));
      }
      catch (IllegalArgumentException ex)
      ...
      nextNullTerminatorIndex = getNextNullTerminator(nullTerminatorIndex, encoding.getCharacterSet());
   ...
   public void setBuffer()
   {
      if (isDirty())
      {
         byte[] mimeTypeBytes    = stringToBytes(Encoding.ISO_8859_1.getCharacterSet(), mimeType   );
         byte[] descriptionBytes = stringToBytes(encoding.getCharacterSet()           , description);
         int    index            = 0;
