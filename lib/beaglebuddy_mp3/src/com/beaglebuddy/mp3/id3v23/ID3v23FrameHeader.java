 ...
 * </table>
 * <p class="beaglebuddy">
 * This class provides methods for reading and writing frame headers in ID3V2.3 frames.  A frame header is 10 bytes long and precedes the
 * {@link com.beaglebuddy.mp3.id3v23.frame_body.ID3v23FrameBody frame body} in an ID3v2.3 {@link com.beaglebuddy.mp3.id3v23.ID3v23Frame frame}.  A frame header has the following fields:
 ...
 *    </thead>
 *    <tbody>
 *       <tr><td class="beaglebuddy">1. </td><td class="beaglebuddy">{@link com.beaglebuddy.mp3.enums.FrameType frameType}</td><td class="beaglebuddy">which of the 74 ID3v2.3 defined types of frame is this the header for.                                                                               </td></tr>
 ...
 *       <tr><td class="beaglebuddy">9. </td><td class="beaglebuddy">uncompressedSize                                     </td><td class="beaglebuddy">optional field set only if the <i>compressed</i> field is true.                                                                                      </td></tr>
 *       <tr><td class="beaglebuddy">10.</td><td class="beaglebuddy">encryptionMethod                                     </td><td class="beaglebuddy">optional field set only if the <i>encrypted</i> field is true.                                                                                       </td></tr>
 *       <tr><td class="beaglebuddy">11.</td><td class="beaglebuddy">groupId                                              </td><td class="beaglebuddy">optional field set only if the <i>belongsToGroup</i> field is true.                                                                                  </td></tr>
...
{
   // class mnemonics
   private static final int  ID3v2_3_FRAME_HEADER_DEFAULT_SIZE                 = 10;
   private static final int  ID3v2_3_FRAME_HEADER_UNCOMPRESSED_SIZE            = 4;
   private static final int  ID3v2_3_FRAME_HEADER_ENCRYPTED_SIZE               = 1;
   private static final int  ID3v2_3_FRAME_HEADER_GROUP_ID_SIZE                = 1;
   private static final int  ID3v2_3_FRAME_HEADER_MAX_SIZE                     = ID3v2_3_FRAME_HEADER_DEFAULT_SIZE + ID3v2_3_FRAME_HEADER_UNCOMPRESSED_SIZE + ID3v2_3_FRAME_HEADER_ENCRYPTED_SIZE + ID3v2_3_FRAME_HEADER_GROUP_ID_SIZE;

   private static final byte ID3v2_3_FRAME_HEADER_TAG_ALTER_PRESERVATION_MASK  = (byte)0x80;
   private static final byte ID3v2_3_FRAME_HEADER_FILE_ALTER_PRESERVATION_MASK = (byte)0x40;
   private static final byte ID3v2_3_FRAME_HEADER_READ_ONLY_MASK               = (byte)0x20;
   private static final byte ID3v2_3_FRAME_HEADER_COMPRESSED_MASK              = (byte)0x80;
   private static final byte ID3v2_3_FRAME_HEADER_ENCRYPTED_MASK               = (byte)0x40;
   private static final byte ID3v2_3_FRAME_HEADER_BELONGS_TO_GROUP_MASK        = (byte)0x20;
   ...
   {
      this.header                = new byte[ID3v2_3_FRAME_HEADER_DEFAULT_SIZE];
   ...
   public ID3v23FrameHeader(InputStream inputStream) throws IOException
   {
      ...
      else
      {
         ...
         byte[] data   = new byte[ID3v2_3_FRAME_HEADER_DEFAULT_SIZE - 1];
         byte[] buffer = new byte[ID3v2_3_FRAME_HEADER_MAX_SIZE];
         ...
         if (inputStream.read(data) == data.length)
         {
            ...
            try
            {
               String s = new String(buffer, 0, 4, Encoding.ISO_8859_1.getCharacterSet());
               frameType = FrameType.getFrameType(s);
            }
            catch (IllegalArgumentException ex)
            {
               invalidFrameId = new String(buffer, 0, 4, Encoding.ISO_8859_1.getCharacterSet());
               ex.printStackTrace();
            ...
            frameBodySize         = ((buffer[4] & 0xFF ) << 24) + ((buffer[5] & 0xFF) << 16) + ((buffer[6] & 0xFF) << 8) + (buffer[7] & 0xFF);
            tagAlterPreservation  = (buffer[8] & ID3v2_3_FRAME_HEADER_TAG_ALTER_PRESERVATION_MASK ) != 0;
            fileAlterPreservation = (buffer[8] & ID3v2_3_FRAME_HEADER_FILE_ALTER_PRESERVATION_MASK) != 0;
            readOnly              = (buffer[8] & ID3v2_3_FRAME_HEADER_READ_ONLY_MASK              ) != 0;
            compressed            = (buffer[9] & ID3v2_3_FRAME_HEADER_COMPRESSED_MASK             ) != 0;
            encrypted             = (buffer[9] & ID3v2_3_FRAME_HEADER_ENCRYPTED_MASK              ) != 0;
            belongsToGroup        = (buffer[9] & ID3v2_3_FRAME_HEADER_BELONGS_TO_GROUP_MASK       ) != 0;

            // if any additional fields are found, then calculate the size of the frame header
            int frameHeaderSize = ID3v2_3_FRAME_HEADER_DEFAULT_SIZE;

            // if the frame is compressed by zlib, get its uncompressed size
            if (compressed)
            {
               byte[] uncompressedSizeData = new byte[ID3v2_3_FRAME_HEADER_UNCOMPRESSED_SIZE];
   ...
   {
      if (frameBodySize < 0)
         throw new IllegalArgumentException("Invalid frame body size, " + frameBodySize + ", in frame " + frameType.getName() + ". It must be > 0.");
   ...
   public void setUncompressedSize(int uncompressedSize)
   {
      if (uncompressedSize < 0)
         throw new IllegalArgumentException("Invalid uncompressed size, " + uncompressedSize + ", in frame " + frameType.getName() + ". It must be >= 0.");
   ...
   {
      if (dirty)
         throw new IllegalStateException("frame " + frameType.getName() + " has been modified and requires setBuffer() to be called before it can be saved.");
   ...
   {
      int frameHeaderSize = ID3v2_3_FRAME_HEADER_DEFAULT_SIZE + (compressed     ? ID3v2_3_FRAME_HEADER_UNCOMPRESSED_SIZE : 0) +
                                                                (encrypted      ? ID3v2_3_FRAME_HEADER_ENCRYPTED_SIZE    : 0) +
                                                                (belongsToGroup ? ID3v2_3_FRAME_HEADER_GROUP_ID_SIZE     : 0);

      byte[] frameIdBytes = ID3v23FrameBodyUtility.stringToBytes(Encoding.ISO_8859_1.getCharacterSet(), frameType.getId());
      ...
      header[8] = (byte)(tagAlterPreservation  ? header[8] | ID3v2_3_FRAME_HEADER_TAG_ALTER_PRESERVATION_MASK  : header[8] & ~ID3v2_3_FRAME_HEADER_TAG_ALTER_PRESERVATION_MASK );
      header[8] = (byte)(fileAlterPreservation ? header[8] | ID3v2_3_FRAME_HEADER_FILE_ALTER_PRESERVATION_MASK : header[8] & ~ID3v2_3_FRAME_HEADER_FILE_ALTER_PRESERVATION_MASK);
      header[8] = (byte)(readOnly              ? header[8] | ID3v2_3_FRAME_HEADER_READ_ONLY_MASK               : header[8] & ~ID3v2_3_FRAME_HEADER_READ_ONLY_MASK              );
      header[9] = (byte)(compressed            ? header[9] | ID3v2_3_FRAME_HEADER_COMPRESSED_MASK              : header[9] & ~ID3v2_3_FRAME_HEADER_COMPRESSED_MASK             );
      header[9] = (byte)(encrypted             ? header[9] | ID3v2_3_FRAME_HEADER_ENCRYPTED_MASK               : header[9] & ~ID3v2_3_FRAME_HEADER_ENCRYPTED_MASK              );
      header[9] = (byte)(belongsToGroup        ? header[9] | ID3v2_3_FRAME_HEADER_BELONGS_TO_GROUP_MASK        : header[9] & ~ID3v2_3_FRAME_HEADER_BELONGS_TO_GROUP_MASK       );

      // if there are any additional fields, then write them to the raw binary header
      if (header.length != ID3v2_3_FRAME_HEADER_DEFAULT_SIZE)
      {
         int index = ID3v2_3_FRAME_HEADER_DEFAULT_SIZE;
         if (compressed)
         {
